import time,lal,lalburst,math,os
from glue.ligolw import lsctables
from pycbc import psd,types
from utils import *
from plots import *

def excess_power(ts_data,min_frequency,max_frequency,psd_segment_length,psd_segment_stride,psd_estimation,window_fraction,tile_fap,nchans,band):
    """
    ...
    nchans,band,flow = check_filtering_settings(sample_rate,min_frequency,max_frequency,channels=nchans,tile_bandwidth=band)
    ...
    print "|- Create filter..."
    filter_bank, fdb = create_filter_bank(fd_psd.delta_f, flow+band/2, band, nchans, fd_psd, spec_corr,min_frequency,max_frequency)
    ...
    print "|-Compute normalization for virtual channel..."
    mu_sq_dict = {}
    t1 = time.time()
    ...
    print "|- Setting up event list and boundaries..."
    ...
    while t_idx_max <= len(ts_data):        
        ...
        tf_map = create_tf_plane(fd_psd,nchans,seg_len,filter_bank)
        ...
        clip_samples = int(psd_segment_length * window_fraction * sample_rate / 2)
        for nc_sum in range(0, int(math.log(nchans, 2)))[::-1]: # nc_sum additional channel adds
            ...
            print "|- Undersampling rate for this level: %f" % (sample_rate/us_rate)
            #======================================================================
...
def create_tf_plane(fd_psd,nchans,seg_len,filter_bank):
    """
...
def create_xml():
    ...
    start_time = LIGOTimeGPS(analysis_start_time or gps_start_time or int(ts_data.start_time))
    end_time = LIGOTimeGPS(analysis_end_time or gps_end_time or int(ts_data.end_time))
